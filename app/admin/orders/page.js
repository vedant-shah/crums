...
function Orders() {
  ...
  const requestPayment = async (tableNumber) => {
  ...
  const setOrderCompleted = async () => {
    ...
    if (!confirmed) return;
    try {
      const response = await fetch(`/api/markaspaid`, {
        method: "POST",
        body: JSON.stringify(selectedOrder.orderIds),
      });
      const data = await response.json();
      if (!data.success) throw data.message;
      toast({
                  ...
                  {currentTab === 2 && (
                    <div className="flex justify-end m-4">
                      <Button
                        variant=""
                        onClick={() =>
                          requestPayment(selectedOrder.tableNumber)
                        }>
                        Request Payment
                      </Button>
                    </div>
